...
const url = 'https://raw.githubusercontent.com/Front-End-Coders-Mauritius/frontend.mu/main/packages/frontendmu-data/data/meetups-raw.json';
...
https.get(url, (response) => {
  ...
  response.on('end', () => {
    ...
    const specifiedMeetup = meetups.find(meetup => meetup.Date === meetupDate);
    ...
    if (specifiedMeetup) {
      ...
      const outputFileName = `meetup_details_${meetupDate}.json`;
      fs.writeFile(outputFileName, JSON.stringify(sessionDetails, null, 2), (err) => {
        if (err) throw err;
        console.log(`Meetup details for ${meetupDate} have been written to ${outputFileName}`);
      });
    } else {
